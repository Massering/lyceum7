...
from data.__all_models.news import News
...
def get_news_by_id(news_id: int):
    ...
    None если новость не найдена.
    """
    session = db_session.create_session()
    return session.query(News).get(news_id)
...
class NewsResource(Resource):
    ...
    NEWS_NOT_FOUND = "News with id: '{0}' is not exists"
    ...
    def get(self, news_id: int):
        ...
        if news is None:
            return jsonify({
                "success": False,
                "error": self.NEWS_NOT_FOUND.format(news_id)
            })
